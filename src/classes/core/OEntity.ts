...
export default class OEntity {
    ...
    private registerEntityClient() {
        ...
        var nme_OEntity = function nme_OEntity($worldIn) {
            ...
            this.wrapped.tasks.addTask(3, AITask("EntityAITempt", 4)(this, 1.5, ModAPI.items[`${entityBreedItem}`].getRef(), 0)); //won't cause a problem as the bread is obtained when the entity is constructed.
        ...
        nme_OEntity.prototype.$isBreedingItem = function (itemstack) {
            return itemstack !== null && itemstack.$getItem() === ModAPI.items[`${entityBreedItem}`].getRef();
        }
        // END CUSTOM ENTITY
    ...
    private registerOEntity() {
        ModAPI.dedicatedServer.appendCode(`globalThis.registerEntityServer("${this.entityID}", "${this.entityName}", "${this.entityModel}", ${this.eggBase}, ${this.eggSpots});`);
        var data = this.registerEntityClient();

        ModAPI.addEventListener("lib:asyncsink", async () => {
            AsyncSink.setFile(`resourcepacks/AsyncSinkLib/assets/minecraft/textures/entity/${this.entityID}.png`, await (await fetch(
                this.entityTexture
            )).arrayBuffer());
            AsyncSink.hideFile(`resourcepacks/AsyncSinkLib/assets/minecraft/textures/entity/${this.entityID}.png.mcmeta`);

            await this.waitForRenderManager();

            AsyncSink.setFile(`resourcepacks/AsyncSinkLib/assets/minecraft/sounds/mob/${this.entityID}/main_sound.ogg`, await (await fetch(
                `${this.entity_sound_main}`
            )).arrayBuffer());
            ...
            AsyncSink.setFile(`resourcepacks/AsyncSinkLib/assets/minecraft/sounds/mob/${this.entityID}/step.ogg`, await (await fetch(
    ...
    public createSpawnegg(texture) {
        ...
        var spawnEgg = new OItem(`Spawn ${this.entityName}`, `spawnegg_${this.entityID}`, 64, texture, () => { }, ($$itemstack, $$world, $$player, $$blockpos) => {
